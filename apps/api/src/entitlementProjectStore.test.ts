...
import { EntitlementLimitError, type EntitlementSnapshot } from '@finaler-draft/entitlements';
...
describe('createEntitlementEnforcedProjectStore: createScreenplay', () => {
  it('free with none: delegates to the base store and claims the empty slot for the new screenplay', async () => {
    ...
    base.createScreenplay = vi.fn(async () => ({ id: screenplayA }));
    ...
    expect(result).toEqual({ id: screenplayA });
  ...
  it('active subscription: creates freely even with existing candidates, and never claims a slot', async () => {
    const base = unusedProjectStore();
    base.createScreenplay = vi.fn(async () => ({ id: screenplayB }));
    const entitlements = entitlementsFixture(
    ...
    expect(result).toEqual({ id: screenplayB });
    expect(entitlements.claimEmptySlot).not.toHaveBeenCalled();
  });
...
// `updateScreenplay` (and this decorator's entitlement gate on it) was deleted along with the
// whole-document `PUT` (collaboration slice 1: the canonical screenplay is a projection of the
// Yjs document `apps/collab` maintains, never a client write over REST -- see
// `progress/collaboration-slice-1.md`). The identical policy this described -- a restricted
// account may write only the screenplay in its editable slot -- is now enforced on the one write
// path that remains, the WebSocket connection itself; see
// `apps/collab/src/authenticate.test.ts`'s equivalent coverage of `resolveConnectionAuthorization`.
