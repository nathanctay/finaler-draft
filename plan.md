...
- **Parentheticals own their parentheses.** Creating a parenthetical wraps the block's existing text in `()`, unless it is already wrapped. Converting a parenthetical to another element strips a leading `(` and trailing `)` if both are present. Once written, the parentheses are ordinary authored text -- selectable, deletable, exportable, indistinguishable from anything else the writer typed -- not characters the editor polices afterward.

The parenthetical rule is the one to be most careful with, because it is the only one that writes characters into the canonical model. The parentheses are authored text once stored, so they round-trip and export like any other text, and the wrap/unwrap must never be applied a second time to a parenthetical that already has them -- including one arriving from FDX import, where the parentheses are already part of the text. A writer can delete just one of the two after creation, leaving `(beat` or `beat)`; converting away from parenthetical strips only when both ends are present, never a lone one, since stripping an unmatched parenthesis would silently edit text the writer typed rather than remove punctuation the editor added.
...
- **Settle client-IP trust against the real deployment, before public launch.** This one must be done with the deployed app in front of you, which is why it cannot be finished from the codebase alone. Better Auth resolves a client address only from a header (it receives a web `Request` and never sees the socket), and `auth.ts` points it at `x-real-ip`. When no address resolves, its fallback is **a single shared rate-limit bucket per path for every client combined** -- confirmed in the installed `api/rate-limiter/index.mjs`, `NO_TRUSTED_IP_KEY`. Sign-in is capped at three requests per ten seconds, so under that fallback any three attempts from anyone lock out the fourth person, and a trivial script holds the login endpoint shut for everyone. That is worse than having no limit at all.

  `apps/api/src/app.ts` now fills `x-real-ip` from the connection when the header is absent, which fixes local development and any direct-connection case. What remains unverified is production: a client that reaches the API directly could set `x-real-ip` itself and rotate it to evade the limit. **Establish first whether the edge proxy overwrites the header** -- `curl -H "x-real-ip: 1.2.3.4" https://<app>/api/health` and see which address the app records. If it overwrites, nothing further is needed. If it passes the value through, set `advanced.ipAddress.trustedProxies` so the header is believed only from the proxy. Do not guess that value from the codebase: too narrow and no address resolves, restoring the shared bucket; too broad and any client can claim to be the proxy, which is no protection at all.
