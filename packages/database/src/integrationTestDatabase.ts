// A deliberate, test-only copy of `apps/api/src/integrationTestDatabase.ts` (byte-identical apart
// from this comment) -- test infrastructure with no production code path, so unlike `createAuth`
// or `checkEntitlement` there is no security or correctness risk in two copies existing, and no
// package boundary in this monorepo currently separates "test helpers" from "app code" in a way
// that would let `apps/api`, `apps/collab`, and this package all depend on one shared copy without
// adding a package for infrastructure this thin. Mirrors the identical reasoning
// `apps/collab/src/integrationTestDatabase.ts` already gave for its own copy.
